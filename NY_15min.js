...
async function Bucle (resolution){
    ...
    let array_acciones = ["AAPL","AMAT","AMD","C","CSCO","CVX","DIS","EBAY","GILD","INTC","JPM","KO","LVS","MET","MOMO","MRK","MS","MU","NKE","NUGT","ORCL","QCOM","SBUX","SNAP","TWTR","UAL","UBER","VZ","WFC","WMT","XOM"];
...
function Peticion(accion,resolution) {
    ...
    return new Promise(resolve => {
        ...
        request('https://finnhub.io/api/v1/indicator?symbol='+accion+'&resolution='+resolution+'&from='+inicio+'&to='+fecha+'&indicator=ADX&timeperiod=14&token=c2qih02ad3ickc1loc60', { json: true }, (err, res, body) => {
            ...
            if(vbs2) { 
                console.log("VBSADX en "+ accion) 
                vbs2.simbolo = operacion.divisas;
                vbs2.fecha = new Date().toLocaleString();
                operacion.jugadas.push(vbs2);
            };
            ...
            if(nbr) {
                ...
                console.log("NBR en "+accion);

                nbr.simbolo = operacion.divisas;
                nbr.fecha = new Date().toLocaleString();

                operacion.jugadas.push(nbr);
            ...
            //     vri.simbolo = operacion.divisas;
            //     vri.fecha = new Date().toLocaleString();

            //     operacion.jugadas.push(vri);

            // };
            // let reversa = Reversa(datos);
            // if(reversa) {

            //     console.log("Reversa en "+accion);

            //     reversa.simbolo = operacion.divisas;
            //     reversa.fecha = new Date().toLocaleString();

            //     operacion.jugadas.push(reversa);
...
async function Inicio(){
    ...
    console.log("Inicializando en 15 minutos")
...
function VBSADX (array_datos){
    ...
    if(MM20 > MM20_10 && MM20_10 > MM20_20 && MM8 > MM8_3 && MM8_3 > MM8_5 && high < high_1 && high_1 < high_2 && adx >= 30 && adx <= 45 ){
    ...
    if(MM20 < MM20_10 && MM20_10 < MM20_20 && MM8 < MM8_3 && MM8_3 < MM8_5 && low > low_1 && low_1 > low_2 && adx >= 30 && adx <= 45 ){
...
function Crear_Json(jugada){
    ...
    return new Promise(resolve => {
        fs.readFile('./Estadisticas/Json/Estadisticas_15min.json', 'utf8', function readFileCallback(err, data){
            if (err){
                console.log(err);
            } else {
            ...
            fs.writeFile('./Estadisticas/Json/Estadisticas_15min.json', json, 'utf8', ()=>{ resolve(console.log("hecho"))}); // write it back 
        }});
    });
