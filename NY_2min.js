...
function Peticion(accion,resolution) {
    ...
    return new Promise(resolve => {
        ...
        request('https://finnhub.io/api/v1/stock/candle?symbol='+accion+'&resolution='+resolution+'&from='+inicio+'&to='+fecha+'&token=c2qih02ad3ickc1loc60', { json: true }, (err, res, body) => {
            ...
            let datos_procesados = Formatear(res.body);
            
            let datos = Procesar(datos_procesados);
...
async function Inicio(){
    ...
    console.log("Inicializando en 2 minutos")
...
function Crear_Json(jugada){
    ...
    return new Promise(resolve => {
        fs.readFile('./Estadisticas/Json/Estadisticas_2min.json', 'utf8', function readFileCallback(err, data){
            if (err){
                console.log(err);
            } else {
            ...
            fs.writeFile('./Estadisticas/Json/Estadisticas_2min.json', json, 'utf8', ()=>{ resolve(console.log("hecho"))}); // write it back 
        }});
    });
